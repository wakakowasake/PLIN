    ...
    <script>
      ...
      function initMap() {
        try {
          ...
          window.applyMapState = function(nextState) {
            userMovedMap = false;
            currentState = Object.assign({}, currentState, nextState || {});
            renderState();
          };

          window.confirmCurrentCenter = confirmCurrentCenter;
          window.focusCandidateByPlaceId = focusCandidateByPlaceId;
...
function buildInjectedMapState(props: {
    mode: MapMode;
    query: string;
    fallbackCenter?: {
        latitude: number;
...
export const InlinePlaceMapPicker = React.forwardRef<InlinePlaceMapPickerHandle, Props>(function InlinePlaceMapPicker(
    ...
    React.useImperativeHandle(ref, () => ({
        adjustZoom(delta: number) {
            ...
            webViewRef.current?.injectJavaScript(
                `window.adjustMapZoom && window.adjustMapZoom(${safeDelta}); true;`
            );
        },
        confirmManualSelection() {
            webViewRef.current?.injectJavaScript(
                'window.confirmCurrentCenter && window.confirmCurrentCenter(); true;'
            );
        },
        focusCandidate(placeId: string) {
            webViewRef.current?.injectJavaScript(
                `window.focusCandidateByPlaceId && window.focusCandidateByPlaceId(${JSON.stringify(placeId)}); true;`
            );
        },
        focusLocation(center: { latitude: number; longitude: number; zoom?: number }) {
            ...
            webViewRef.current?.injectJavaScript(
                `window.focusLocation && window.focusLocation(${latitude}, ${longitude}, ${zoom}); true;`
            );
        }
    }), []);
    ...
    React.useEffect(() => {
        if (!apiKey || !isMapReady || !webViewRef.current) {
            return;
        }

        const payload = buildInjectedMapState({
            mode,
            query,
            fallbackCenter,
            fallbackQuery,
        ...
        webViewRef.current.injectJavaScript(
            `window.applyMapState && window.applyMapState(${payload}); true;`
        );
    }, [
        apiKey,
        candidates,
        ...
        <View style={styles.container}>
            {apiKey ? (
                <WebView
                    ...
                    originWhitelist={['*']}
                    onError={() => {
                        const message = '지도를 불러오지 못했어요. 잠시 후 다시 시도해 주세요.';
                        setLocalError(message);
                        onMapError(message);
                    }}
                    onMessage={(event) => {
