...
/**
 * Add list item (wrapper for both shopping and checklist)
 * @param {string} type - 'shopping' or 'check'
 */
...
export function renderListInModal() {
    ...
    // ✅ Phase 5.6 Step 3: onclick 제거, data-* 속성 대신 사용
    container.innerHTML = list.map((item, index) => `
        <div class="flex items-center gap-3 p-1 group">
            <button class="toggle-check flex-shrink-0 text-gray-400 hover:text-primary transition-colors" data-type="${activeListType}" data-index="${index}">
                <span class="material-symbols-outlined text-2xl">${item.checked ? 'check_box' : 'check_box_outline_blank'}</span>
            </button>
            <div class="flex-1 min-w-0">
                <div class="flex flex-col">
                    <input type="text" value="${item.text}" 
                        class="list-item-input w-full bg-transparent border-none text-text-main dark:text-white outline-none focus:ring-0 ${item.checked ? 'line-through text-gray-400' : ''}"
                        data-type="${activeListType}" data-index="${index}">
                    ${item.location ? `
            ...
            <button class="delete-list-item text-gray-300 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity" data-type="${activeListType}" data-index="${index}">
                <span class="material-symbols-outlined">delete</span>
            </button>
        </div>
    `).join('');

    // ✅ Phase 5.6 Step 3: 이벤트 위임 (Event Delegation)
    // 토글 체크
    container.querySelectorAll('.toggle-check').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const type = e.currentTarget.dataset.type;
            const index = parseInt(e.currentTarget.dataset.index);
            toggleListCheck(type, index);
        });
    });
    ...
    // 삭제 버튼
    container.querySelectorAll('.delete-list-item').forEach(btn => {
        btn.addEventListener('click', (e) => {
            const type = e.currentTarget.dataset.type;
            const index = parseInt(e.currentTarget.dataset.index);
...
// ✅ Phase 5.6 Step 3: updateListItemText 제거 (renderListInModal에서 직접 처리)
