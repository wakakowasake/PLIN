...
import { buildExpenseDetailState, deleteExpenseFromTravelData } from '../features/expenses/expense-detail-flow.js';
import { calculateSplitAmount } from '../features/expenses/expense-helpers.js';
...
export function ensureExpenseDetailModal() {
    if (!document.getElementById('expense-detail-modal')) {
        ...
        modal.innerHTML = `
            <div class="modal-surface-card expense-detail-shell bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl flex flex-col max-h-[85vh] modal-slide-in" onclick="event.stopPropagation()">
                <div class="modal-surface-header expense-detail-header p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center bg-gradient-to-r from-green-50 to-emerald-50 dark:from-gray-800 dark:to-gray-800">
                    <h3 class="text-lg font-bold text-gray-800 dark:text-white flex items-center gap-2">
                        <span class="material-symbols-outlined text-green-600">payments</span>
                        지출 상세
                    </h3>
                    <button onclick="window.closeExpenseDetailModal()" aria-label="지출 상세 닫기"
                        class="modal-icon-button text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
                        <span class="material-symbols-outlined">close</span>
                    </button>
                </div>
                
                <div class="modal-surface-body expense-detail-body p-4 overflow-y-auto custom-scrollbar flex-1">
                     <div class="expense-detail-summary bg-card-light dark:bg-card-dark rounded-xl p-6 mb-6 text-center border border-gray-100 dark:border-gray-700 shadow-sm relative overflow-hidden group">
                    ...
                    <div class="expense-split-card bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 border border-gray-200 dark:border-gray-700/50">
                        ...
                        <div class="flex gap-2 mb-4">
                            <div class="relative flex-1">
                                ...
                                <input type="number" id="split-people-count" value="1" min="1"
                                    class="modal-text-input w-full pl-12 pr-4 py-3 rounded-xl border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-800 dark:text-white focus:ring-2 focus:ring-primary/50 focus:border-primary transition-all">
                            </div>
                            <button onclick="window.calculateSplit()" class="modal-primary-button bg-primary hover:bg-primary-dark text-white px-6 rounded-xl font-bold transition-all shadow-md hover:shadow-lg active:scale-95 whitespace-nowrap">
                                계산하기
                            </button>
                        </div>
                        <div id="split-result" class="expense-split-result hidden bg-white dark:bg-gray-800 rounded-lg p-4 text-center border border-primary/20 shadow-sm animate-fade-in">
...
function renderExpenseDetailContent() {
    const { totalExpense, expensesByDay } = buildExpenseDetailState(travelData);
    ...
    if (expensesByDay.length === 0) {
        ...
        dayListEl.innerHTML = expensesByDay.map((dayData, idx) => `
            <div class="expense-day-card bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-200 dark:border-gray-700">
                <div class="flex justify-between items-center mb-3">
                    <div class="flex items-center gap-2">
                        <h5 class="font-bold text-gray-800 dark:text-white">${dayData.date}</h5>
                        <button onclick="window.addExpenseFromDetail(${dayData.dayIdx})" class="expense-day-add-btn text-xs bg-primary/10 hover:bg-primary/20 text-primary px-2 py-1 rounded transition-colors font-bold flex items-center gap-1">
                ...
                <div class="space-y-2">
                    ${dayData.expenses.map(exp => `
                        <div class="expense-line-item flex justify-between items-center text-sm bg-gray-50 dark:bg-gray-900 p-2 rounded-lg group">
...
export function calculateSplit() {
    ...
    const perPerson = calculateSplitAmount(total, peopleCount);
...
export function deleteExpenseFromDetail(dayIdx, itemIdx, expIdx) {
    ...
    const item = deleteExpenseFromTravelData(travelData, dayIdx, itemIdx, expIdx);
    if (!item) return;
