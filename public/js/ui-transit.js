import {
    ...
    isEditing, isReadOnlyMode, currentTripId
} from './state.js';
import {
    parseTimeStr,
...
import {
    buildAirplaneRouteUpdate,
    buildGenericTransitRouteUpdate,
    buildInsertedTransitItem,
    buildSavedTransitItem,
    parseTransitDurationValue,
    resolveTransitEndTime
} from './features/transit/transit-item-helpers.js';
import {
    buildEkispertRouteItems,
    buildGoogleRouteSummaryItem,
    buildGoogleRouteSummaryMeta,
    buildGoogleRouteTiming,
    buildGoogleTransitDetailedStep,
    buildGoogleWalkingDetailedStep,
    getAdjustedNextItemTime
} from './features/transit/transit-route-helpers.js';
import {
    calculateArrivalTimeValue,
    calculateFlightDurationValue,
    createAirportSuggestionState,
    formatAirportSelectionValue,
    getAirportSelectionIndex,
    getAirportSuggestions
} from './features/transit/flight-helpers.js';

import { openUserProfile } from './ui/profile.js';
import { renderMemoriesList } from './ui/memories.js';
import { auth, storage } from './firebase.js';
import { ref, uploadBytes, getDownloadURL } from 'https://www.gstatic.com/firebasejs/10.8.0/firebase-storage.js';
import { Z_INDEX, majorAirports } from './ui/constants.js';
...
function buildDirectionsLocationValue(value) {
    if (hasFiniteCoords(value)) {
...
function buildGoogleMapsDirectionsUrl(origin, destination, mode = 'transit') {
    const originValue = buildDirectionsLocationValue(origin);
    const destinationValue = buildDirectionsLocationValue(destination);
    if (!originValue || !destinationValue) return '';

    return `https://www.google.com/maps/dir/?api=1&origin=${encodeURIComponent(originValue)}&destination=${encodeURIComponent(destinationValue)}&travelmode=${encodeURIComponent(mode)}`;
}

function normalizeDirectionsStep(step = {}) {
    return {
        ...step,
        instructions: step.instructions ?? step.html_instructions ?? ''
    };
}
...
function normalizeDirectionsRoute(route = {}) {
    return {
        ...
        legs: Array.isArray(route.legs)
            ? route.legs.map((leg) => ({
                ...leg,
                steps: Array.isArray(leg.steps) ? leg.steps.map(normalizeDirectionsStep) : []
            }))
            : []
    };
}

async function fetchQuickTransitRoutes({ origin, destination, departureTime, dayDate, mode }) {
    if (!BACKEND_URL) {
        throw new Error('경로 검색 서버 주소가 설정되지 않았습니다.');
    }

    const params = new URLSearchParams();
    if (hasFiniteCoords(origin)) {
        params.set('originLat', String(Number(origin.lat)));
        params.set('originLng', String(Number(origin.lng)));
    } else if (origin) {
        params.set('originQuery', String(origin));
    }

    if (hasFiniteCoords(destination)) {
        params.set('destinationLat', String(Number(destination.lat)));
        params.set('destinationLng', String(Number(destination.lng)));
    } else if (destination) {
        params.set('destinationQuery', String(destination));
    }

    if (departureTime) {
        params.set('departureTime', String(departureTime));
    }
    if (dayDate) {
        params.set('dayDate', String(dayDate));
    }
    if (mode) {
        params.set('mode', String(mode));
    }

    const payload = await fetchBackendJson(`/routes/quick-transit?${params.toString()}`);

    if (!Array.isArray(payload?.routes) || payload.routes.length === 0) {
        throw new Error(payload?.message || '자동 추천 경로를 불러오지 못했습니다.');
    }

    return payload.routes.map(normalizeDirectionsRoute);
}
...
function buildEstimatedRouteMeta(distanceMeters, preferredMode = 'transit') {
    ...
    if (preferredMode === 'walking' || distanceMeters <= 1000) {
        const durationMins = Math.max(1, Math.ceil(distanceMeters / 80));
        return {
            title: '도보로 이동',
    ...
    if (preferredMode === 'driving') {
        const durationMins = Math.max(5, Math.ceil(distanceMeters / (25000 / 60)));
        return {
            title: '차량으로 이동',
            icon: 'directions_car',
            tag: '차량',
    ...
    if (distanceMeters <= 5000) {
        durationMins = Math.ceil(distanceMeters / 120);
    } else if (distanceMeters <= 15000) {
        durationMins = Math.ceil(distanceMeters / (9000 / 60));
    } else if (distanceMeters <= 40000) {
        durationMins = Math.ceil(distanceMeters / (13000 / 60));
    } else {
        durationMins = Math.ceil(distanceMeters / (50000 / 60));
    }
...
function formatDurationFromMinutes(durationMins) {
    const hours = Math.floor(durationMins / 60);
    const minutes = durationMins % 60;
    return hours > 0 ? `${hours}시간 ${minutes}분` : `${minutes}분`;
}

function buildEstimatedTransitSummary(origin, destination, preferredMode = 'transit') {
    ...
    const distanceMeters = calculateStraightDistance(normalizedOrigin, normalizedDestination);
    ...
    return {
        ...meta,
        distanceMeters,
        durationStr: formatDurationFromMinutes(meta.durationMins)
    };
}

function getRouteReferenceStartTime(timeline, insertIdx, fallbackItem) {
    let startTime = '';
    const referenceItem = (typeof insertIdx === 'number' && insertIdx >= 0) ? timeline[insertIdx] : fallbackItem;

    if (!referenceItem) return startTime;

    if (referenceItem.isTransit && referenceItem.transitInfo?.end) {
        return referenceItem.transitInfo.end;
    }

    if (referenceItem.time) {
        const prevTimeMinutes = parseTimeStr(referenceItem.time);
        if (prevTimeMinutes !== null) {
            const prevDuration = referenceItem.duration || 30;
            return minutesTo24Hour(prevTimeMinutes + prevDuration);
        }
    }
...
function buildEstimatedTransitItem({ origin, destination, preferredMode, timeline, insertIdx, fallbackItem }) {
    const summary = buildEstimatedTransitSummary(origin, destination, preferredMode);
    if (!summary) return null;

    const startTime = getRouteReferenceStartTime(timeline, insertIdx, fallbackItem);
    let endTime = '';
    if (startTime) {
        const startMinutes = parseTimeStr(startTime);
        if (startMinutes !== null) {
            endTime = minutesTo24Hour(startMinutes + summary.durationMins);
        }
    }

    return {
        time: summary.durationStr,
        duration: summary.durationMins,
        title: summary.title,
        location: '',
        icon: summary.icon,
        tag: summary.tag,
        isTransit: true,
        image: null,
        note: `직선거리: ${Math.round(summary.distanceMeters)}m (추정 경로)`,
        fixedDuration: true,
...
export function addTransitItem(index, type, dayIndex = currentDayIndex) {
    ...
    const newItem = buildInsertedTransitItem(type, day.timeline, index);
...
export function fetchTransitTime() {
    ...
    fetchQuickTransitRoutes({
        origin,
        destination,
        departureTime: startTimeInput.value,
        ...
        if (!durationSec || !durationText) {
            throw new Error('경로 시간 정보를 찾지 못했습니다.');
        }
        ...
        console.warn('Quick transit route failed:', error);

        const estimated = buildEstimatedTransitSummary(origin, destination, mode);
        if (estimated) {
            const startMins = h * 60 + m;
            const endMins = startMins + estimated.durationMins;
            const eh = Math.floor(endMins / 60) % 24;
            const em = endMins % 60;
            ...
            const noteInput = document.getElementById('transit-note');
            if (!noteInput.value) {
                noteInput.value = `${estimated.tag} 추정 경로: ${estimated.durationStr}`;
            }

            calculateTransitDuration();
            alert(`실시간 경로를 불러오지 못해 추정 시간을 넣었습니다.\n예상 소요시간: ${estimated.durationStr}`);
            return;
        }

        const directionsUrl = buildGoogleMapsDirectionsUrl(origin, destination, mode);
        const shouldOpenExternal = directionsUrl
            ? confirm("앱 안에서 경로를 불러오지 못했습니다.\n구글 지도에서 길찾기를 열어볼까요?")
            : false;
        ...
        alert("경로를 찾을 수 없습니다. 시간을 직접 입력해주세요.");
    });
}
...
export function saveTransitItem() {
    ...
    if (!end) {
        ...
        end = resolveTransitEndTime(start, nextItem);
        document.getElementById('transit-end-time').value = end;
    }

    const durationStr = calculateTransitDuration();
    const durationMins = parseTransitDurationValue(durationStr);
    ...
    if (isTransitEditing) {
        ...
        const newItem = buildSavedTransitItem(transitInputType, {
            start,
            end,
...
export function openTransitDetailModal(item, index, dayIndex) {
    ...
    // [User Request] 자동 추천 경로 항목은 수정 버튼 숨기기
...
export function deleteCurrentTransitItem() {
    ...
    if (itemIndex !== null && targetDayIndex !== null) {
        ...
        if (modal) {
            modal.classList.add('modal-z-confirm');
            modal.classList.remove('hidden');
        }
    }
}
...
export async function addFastestTransitItem(insertIndexOverride = null, dayIndexOverride = null) {
    ...
    // ✅ [FIX] 명시적 파라미터 사용 (selectAddType에서 전달)
    const effectiveInsertIndex = insertIndexOverride !== null && insertIndexOverride !== undefined 
        ? insertIndexOverride 
        : insertingItemIndex;
    const effectiveDayIndex = dayIndexOverride !== null && dayIndexOverride !== undefined
        ? dayIndexOverride
        : targetDayIndex;
    
    if (effectiveDayIndex === null || effectiveDayIndex === -1 || !travelData.days[effectiveDayIndex]) {
        alert("날짜 정보를 찾을 수 없습니다.");
        return;
    }

    const timeline = travelData.days[effectiveDayIndex].timeline;
    
    // ✅ [Fix] effectiveInsertIndex가 null/undefined인 경우도 처리
    const insertIdx = (effectiveInsertIndex !== null && effectiveInsertIndex !== undefined) 
        ? Number(effectiveInsertIndex) 
        : -1;
    
    // ✅ [Fix] insertIdx가 -1이면 사용자에게 가이드 제공
    if (insertIdx === -1) {
    ...
    const btn = document.querySelector('#add-selection-modal button[data-action="add-fastest-item"]');
    ...
    try {
        ...
        const getStoredCountryCode = (item) => {
            if (item.countryCode) return item.countryCode;
            if (Array.isArray(item.address_components)) {
                const country = item.address_components.find((entry) => Array.isArray(entry?.types) && entry.types.includes('country'));
                if (country?.short_name) return country.short_name;
            }
            return null;
        };

        // 앞뒤 장소의 국가 확인
        const prevCountry = getStoredCountryCode(prevItem);
        const nextCountry = getStoredCountryCode(nextItem);
        ...
        if (isTargetRegion) {
            const estimatedItem = buildEstimatedTransitItem({
                origin,
                destination,
                preferredMode: 'transit',
                timeline,
                insertIdx,
                fallbackItem: prevItem
            });

            if (estimatedItem) {
                timeline.splice(insertIdx + 1, 0, estimatedItem);
                reorderTimeline(effectiveDayIndex);
                closeAddModal();
                return;
            }
        ...
        const tripDateStr = travelData.days[effectiveDayIndex].date;
        if (tripDateStr) {
        ...
        let resultRoutes = null;
        let searchMode = null;

        if (!resultRoutes) {
            try {
                resultRoutes = await fetchQuickTransitRoutes({
                    origin,
                    destination,
                    departureTime: `${String(departureTime.getHours()).padStart(2, '0')}:${String(departureTime.getMinutes()).padStart(2, '0')}`,
                    dayDate: tripDateStr || ''
                });
                searchMode = 'transit';
            } catch (error) {
                console.warn('Quick transit route search failed:', error);
            }
        }

        if (!resultRoutes) {
            logger.log("🕒 서버 경로 탐색 실패, 추정 경로로 대체");
            const estimatedItem = buildEstimatedTransitItem({
                origin,
                destination,
                preferredMode: 'transit',
            ...
            if (estimatedItem) {
                ...
                reorderTimeline(effectiveDayIndex);
                closeAddModal();
                alert("실시간 경로를 불러오지 못해 추정 경로를 추가했어요.");
                return;
            }
        }
        ...
        if (resultRoutes) {
            closeAddModal();
            setTimeout(() => openRouteSelectionModal(resultRoutes, insertIdx, searchMode), 50);
        } else {
    ...
    routes.forEach((route, idx) => {
        ...
        if (transitSteps.length > 0) {
            transitSteps.forEach(step => {
                ...
                const rawLineName = step.transit?.line?.short_name || step.transit?.line?.name || '';
                const lineName = translateLine(rawLineName);
...
function processSelectedRoute(route, insertIdx) {
    ...
    // 상세 경로 정보 생성 (펼쳐질 내용)
    if (!summaryMeta.hasTransit) {
        detailedSteps.push(buildGoogleWalkingDetailedStep(summaryMeta.totalDuration || "시간 미정", "경로 상세 정보 없음"));
    } else {
        for (const step of steps) {
            if (step.travel_mode === 'TRANSIT' && step.transit) {
                detailedSteps.push(buildGoogleTransitDetailedStep(step));
            } else if (step.travel_mode === 'WALKING') {
                const stepDuration = step.duration?.text || "";
                let instructions = step.instructions === undefined || step.instructions === null
                    ? "도보로 이동"
                    : String(step.instructions);
                const div = document.createElement("div");
                div.innerHTML = instructions;
                instructions = div.textContent || div.innerText || "도보로 이동";
                detailedSteps.push(buildGoogleWalkingDetailedStep(stepDuration, instructions));
            }
        }
    }
    ...
    const { routeStartTime, routeEndTime } = buildGoogleRouteTiming(
        timelineArr,
        insertIdx,
        summaryMeta.totalMinutes
    );

    // 고유한 그룹 ID 생성 (자동 추천 경로 항목 식별용)
    const routeGroupId = `google_route_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const summaryItem = buildGoogleRouteSummaryItem({
        summaryTitle: summaryMeta.summaryTitle,
        summaryIcon: summaryMeta.summaryIcon,
        summaryTag: summaryMeta.summaryTag,
        totalDuration: summaryMeta.totalDuration,
        totalDistance: summaryMeta.totalDistance,
        totalMinutes: summaryMeta.totalMinutes,
        detailedSteps,
        routeGroupId,
        routeStartTime,
        routeEndTime
    ...
    if (insertIdx >= 0 && insertIdx < timelineArr.length) {
        ...
        const adjustedNextTime = getAdjustedNextItemTime(prevItem, nextItem, leg?.duration?.value || leg?.duration);
        if (adjustedNextTime) {
            nextItem.time = adjustedNextTime;
        }
    }
...
export async function openRouteModal() {
    ...
    let routeMap = window.routeMap || null;
    let routePolyline = window.routePolyline || null;
    let routeMarkers = Array.isArray(window.routeMarkers) ? window.routeMarkers : [];
    ...
    window.routePolyline = routePolyline;
        ...
        try {
            ...
            if (pos) {
                ...
                marker.addListener('click', () => {
                    const safeTitle = escapeHtml(item.title || '장소');
                    const safeLocation = escapeHtml(item.location || '');
                    const safeTime = escapeHtml(item.time || '');
                    sharedInfoWindow.setContent(`
                        <div style="padding: 8px; min-width: 150px;">
                            <h4 style="font-weight: bold; font-size: 14px; margin-bottom: 4px;">${safeTitle}</h4>
                            <p style="font-size: 12px; color: #666; margin-bottom: 8px;">${safeLocation}</p>
                            <span style="display: inline-block; background: #fff2ec; color: #e84500; border: 1px solid #ffd8c7; font-size: 12px; font-weight: bold; padding: 4px 8px; border-radius: 4px;">${safeTime}</span>
                        </div>
                    `);
                ...
                if (lastPlacePos && transitBuffer.length > 0) {
                    ...
                    for (let j = 0; j < count; j++) {
                        ...
                        tMarker.addListener('click', () => {
                            const safeTransitIcon = escapeHtml(tItem.icon || 'directions_transit');
                            const safeTransitTitle = escapeHtml(tItem.title || '이동');
                            const safeTransitTime = escapeHtml(tItem.time || '');
                            const safeTransitNote = escapeHtml(tItem.note || '');
                            sharedInfoWindow.setContent(`
                                <div style="padding: 8px; min-width: 150px;">
                                    <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 4px;">
                                        <span class="material-symbols-outlined" style="color: #774b00;">${safeTransitIcon}</span>
                                        <h4 style="font-weight: bold; font-size: 14px;">${safeTransitTitle}</h4>
                                    </div>
                                    ${safeTransitTime ? `<span style="display: inline-block; background: #fff2ec; color: #e84500; border: 1px solid #ffd8c7; font-size: 12px; font-weight: bold; padding: 4px 8px; border-radius: 4px; margin-top: 4px;">${safeTransitTime}</span>` : ''}
                                    ${safeTransitNote ? `<p style="font-size: 12px; color: #666; margin-top: 4px;">📝 ${safeTransitNote}</p>` : ''}
                                </div>
                            `);
...
export function viewRouteDetail(index, dayIndex = currentDayIndex, isRouteEditMode = false) {
    ...
    // 자동 추천 경로 여부 확인 (routeGroupId가 있으면 자동 추천 경로)
                ...
                <div class="bg-white dark:bg-card-dark rounded-xl p-6 shadow-sm border border-gray-200 dark:border-gray-700">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" id="route-edit-duration" value="${typeof item.duration === 'number' ? item.duration : (parseTransitDurationValue(item.duration) || 30)}" placeholder="30" min="1" class="w-full border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm" oninput="updateTransitArrivalTime()">
                        </div>
                    </div>
                </div>
            `;
    ...
    const detailSectionsHTML = `
            ...
            <div>
                ...
                <div class="flex justify-between items-center border-t border-gray-100 dark:border-gray-700 pt-3">
                    ...
                    <div class="relative w-40">
                        ...
                        <input id="route-total-budget" type="text" class="w-full pl-8 pr-2 py-1.5 bg-gray-50 dark:bg-gray-900 border-none rounded-lg text-right font-bold text-xl text-primary outline-none cursor-default" readonly value="${(item.expenses || []).reduce((sum, exp) => sum + (Number(exp.amount) || 0), 0).toLocaleString()}">
                    </div>
                </div>
            </div>
        ...
        const isPDF = safeType === 'application/pdf' || String(att?.url || '').toLowerCase().endsWith('.pdf');
        const safeUrl = isPDF
        ...
        return `
                            <div class="relative group">
                                ${isPDF ? `
                                <a href="${escapedUrl}" target="_blank" rel="noopener noreferrer" class="block aspect-square bg-gray-100 dark:bg-gray-800 rounded-lg flex flex-col items-center justify-center hover:bg-gray-200 dark:hover:bg-gray-700 transition-colors">
                                    <span class="material-symbols-outlined text-4xl text-red-500 mb-2">picture_as_pdf</span>
                                    <span class="text-xs text-gray-600 dark:text-gray-400 px-2 text-center truncate w-full">${escapeHtml(att?.name || 'PDF')}</span>
                                </a>
                                ` : `
                                <img src="${escapedUrl}" alt="${escapeHtml(att?.name || 'Attachment')}" class="w-full aspect-square object-cover rounded-lg cursor-pointer" onclick="window.open('${escapedUrl}', '_blank', 'noopener')">
                                `}
...
window.enableTransitNoteEdit = function () {
    ...
    const handleBlur = () => {
        ...
        window.updateRouteItemNote?.(textarea.value); // 자동 저장
...
window.openRouteExpenseModal = function () {
    ...
    if (window.openExpenseModal) {
        ...
        if (modal) {
            modal.classList.remove('hidden');
            modal.classList.add('modal-z-max');
            if (modal.parentNode !== document.body) document.body.appendChild(modal);
        }
        if (Object.prototype.hasOwnProperty.call(window, 'isAddingFromDetail')) window.isAddingFromDetail = false;
    }
...
window.saveRouteExpense = function () {
    ...
    if (window.selectedShoppingItemIndex !== null && travelData.shoppingList && travelData.shoppingList[window.selectedShoppingItemIndex]) {
        ...
        if (typeof window.renderLists === 'function') {
            window.renderLists(); // 쇼핑 리스트 UI 업데이트
        }
    }
...
window.handleRouteAttachmentUpload = async function (input) {
    ...
    const timeline = travelData.days[targetDayIndex].timeline;
    const item = typeof currentRouteItemIndex === 'number' ? timeline[currentRouteItemIndex] : null;

    if (!item || !item.isTransit) return;
    if (!currentTripId) {
        alert('여행 ID를 찾을 수 없습니다.');
        return;
    }
    ...
    if (file.size > maxSize) {
        alert(`파일 크기는 ${isImageFile ? '5MB' : '10MB'} 이하여야 합니다.`);
        input.value = '';
        return;
    }
    ...
    try {
        ...
        const storagePath = `attachments/${currentTripId}/${Date.now()}_${sanitizedName}`;
...
window.saveRouteItem = function () {
    ...
    if (isAirplane) {
        const update = buildAirplaneRouteUpdate({
            departure: document.getElementById('route-edit-departure')?.value || '',
            arrival: document.getElementById('route-edit-arrival')?.value || '',
            departureTime: document.getElementById('route-edit-departure-time')?.value || '',
            arrivalTime: document.getElementById('route-edit-arrival-time')?.value || '',
            duration: document.getElementById('route-edit-duration')?.value || '30분',
            flightNumber: document.getElementById('route-edit-flight-number')?.value || '',
            bookingRef: document.getElementById('route-edit-booking-ref')?.value || '',
            terminal: document.getElementById('route-edit-terminal')?.value || '',
            gate: document.getElementById('route-edit-gate')?.value || ''
        });

        item.title = update.title;
        item.time = update.time;
        item.duration = update.duration;
        item.flightInfo = update.flightInfo;
    } else {
        const update = buildGenericTransitRouteUpdate({
            title: document.getElementById('route-edit-title')?.value || '',
            durationMinutes: document.getElementById('route-edit-duration')?.value,
            note: document.getElementById('route-detail-note')?.value,
            transitStart: item.transitInfo?.start
        });

        item.title = update.title;
        item.duration = update.duration;
        item.time = update.time;
        if (item.transitInfo && update.transitEnd) {
            item.transitInfo.end = update.transitEnd;
        }
...
// 공항 자동완성을 위한 전역 상태
window.airportSuggestionState = createAirportSuggestionState();

// 공항 자동완성 필터링
window.filterAirports = function (type, query) {
    const results = getAirportSuggestions(query);
    ...
    // 결과 저장
    window.airportSuggestionState[type].results = results;
    window.airportSuggestionState[type].selectedIndex = 0;
...
function renderAirportSuggestions(type) {
    ...
    suggestionsDiv.innerHTML = state.results.map((airport, idx) => {
        ...
        const safeCode = escapeJsString(airport.code || '');
        const safeName = escapeJsString(airport.name || '');
        return `
            <div class="px-3 py-2 cursor-pointer text-sm ${isSelected ? 'bg-primary/20 dark:bg-primary/30' : 'hover:bg-gray-100 dark:hover:bg-gray-700'}" 
                onclick="event.stopPropagation(); window.selectAirport('${type}', '${safeCode}', '${safeName}')">
                <span class="font-bold">${escapeHtml(airport.code || '')}</span> | ${escapeHtml(airport.name || '')}
                <span class="text-xs text-gray-500 ml-2">${escapeHtml(airport.city || '')}</span>
            </div>
        `;
    }).join('');
}
...
window.showAirportSuggestions = function (type) {
    ...
    if (input && input.value) {
        window.filterAirports(type, input.value);
    }
};
...
window.handleAirportKeydown = function (event, type) {
    ...
    switch (event.key) {
        ...
        case 'ArrowUp':
            ...
            state.selectedIndex = getAirportSelectionIndex(state.selectedIndex, event.key, state.results.length);
            renderAirportSuggestions(type);
            break;
        ...
        case 'Enter': {
            ...
            if (selected) {
                window.selectAirport(type, selected.code, selected.name);
            }
            break;
        }
...
window.selectAirport = function (type, code, name) {
    ...
    if (input) {
        input.value = formatAirportSelectionValue(code, name);
    }
    if (suggestionsDiv) {
        suggestionsDiv.classList.add('hidden');
    }
...
window.calculateArrivalTime = function () {
    ...
    const arrivalTime = calculateArrivalTimeValue(departureTime, duration);
    if (arrivalTime) {
...
window.calculateFlightDuration = function () {
    ...
    const durationText = calculateFlightDurationValue(departureTime, arrivalTime);
    if (durationText) {
...
async function getEkispertRoute(fromItem, toItem) {
    try {
        ...
        // 좌표 기반 검색만 허용합니다. 이름 기반 레거시 경로는 1차에서 정리합니다.
        ...
        const data = await fetchBackendJson(`/ekispert-proxy?${params.toString()}`);
        ...
        return buildEkispertRouteItems({
            route,
            lines,
            points,
            fromItem,
