...
let wizardAutocompleteInput = null;
let tripInfoAutocomplete;
let AdvancedMarkerCtor = null;
let hasAttemptedGoogleMapsLoad = false;
let googleMapsUnavailableReason = "";
...
function extractPhotoReferenceFromGoogleUrl(photoUrl = '') {
...
function getPhotoReference(photo, maxWidth = 1600, maxHeight = 1200) {
    if (!photo) return null;
    if (photo.photo_reference) return photo.photo_reference;

    if (typeof photo.getPhotoReference === 'function') {
        try {
            const ref = photo.getPhotoReference();
            if (ref) return ref;
        } catch (e) {
            logger.warn('getPhotoReference failed', e);
        }
    }

    if (typeof photo.getUrl === 'function') {
        try {
            const url = photo.getUrl({ maxWidth, maxHeight });
            return extractPhotoReferenceFromGoogleUrl(url);
        } catch (e) {
            logger.warn('photo.getUrl failed', e);
        }
    }
...
function buildGooglePhotoProxyUrl(photo, maxWidth = 1600) {
    const ref = getPhotoReference(photo, maxWidth, Math.round(maxWidth * 0.75));
    if (!ref || !BACKEND_URL) return null;
    return `${BACKEND_URL}/google-photo-proxy?reference=${encodeURIComponent(ref)}&maxwidth=${maxWidth}`;
}

function normalizeLegacyGooglePhotoUrl(url, maxWidth = 1600) {
    const ref = extractPhotoReferenceFromGoogleUrl(url);
    if (!ref || !BACKEND_URL) return url;
    return `${BACKEND_URL}/google-photo-proxy?reference=${encodeURIComponent(ref)}&maxwidth=${maxWidth}`;
}

function attachMarkerClickListener(marker, handler) {
...
// 🔄 [DI Pattern] Injected callbacks for React migration
let injectedHandlers = {
    updateMeta: null,
    renderItinerary: null
};

export function injectMapHandlers(handlers) {
    injectedHandlers = { ...injectedHandlers, ...handlers };
}
...
export function gm_authFailure() {
    googleMapsUnavailableReason = "Google Maps JavaScript API 인증에 실패했습니다.";
    console.error("Google Maps API Authentication Error. Please check your browser API key settings in Google Cloud Console.");
}
...
export function setupWizardAutocomplete() {
    ...
    if (wizardAutocomplete && wizardAutocompleteInput === input) return;
    ...
    wizardAutocomplete.addListener("place_changed", () => {
        ...
        if (place.photos && place.photos.length > 0) {
            // [Fix] 403 Forbidden 및 domain restriction 해결을 위해 proxy 사용
            const photoUrl = buildGooglePhotoProxyUrl(place.photos[0], 1600);
            if (photoUrl) newTripDataTemp.mapImage = photoUrl;
        }

        document.dispatchEvent(new CustomEvent('plin:new-trip-place-selected', {
            detail: {
                name: newTripDataTemp.locationName,
                address: newTripDataTemp.address
            }
        }));
...
export function setupTripInfoAutocomplete() {
    ...
    tripInfoAutocomplete.addListener("place_changed", () => {
        ...
        if (titleInput && !titleInput.value.trim()) {
            const nextTitle = truncateTripTitle(place.name, TRIP_TITLE_MAX_LENGTH);
            titleInput.value = nextTitle;
            titleInput.dispatchEvent(new Event('input', { bubbles: true }));
            if (injectedHandlers.updateMeta) injectedHandlers.updateMeta('title', nextTitle);
        }

        // 장소 필드 및 subInfo 업데이트 (UI 반응용)
        if (input) input.value = place.formatted_address;
        if (injectedHandlers.updateMeta) {
            ...
            injectedHandlers.updateMeta('subInfo', `${place.formatted_address} • ${dateStr}`);
        }
        ...
        if (place.photos && place.photos.length > 0) {
            ...
            if (photoUrl && injectedHandlers.updateMeta) {
                injectedHandlers.updateMeta('mapImage', photoUrl);
                injectedHandlers.updateMeta('defaultMapImage', photoUrl);
            }
        }
        ...
        fetchUnsplashImage(searchQuery).then(url => {
            if (url && injectedHandlers.updateMeta) {
                injectedHandlers.updateMeta('mapImage', url);
                injectedHandlers.updateMeta('defaultMapImage', url);
        ...
        if (mapMarker) {
            // [Fix] Compatibility: AdvancedMarkerElement vs deprecated Marker
            // AdvancedMarkerElement uses 'position' property, Marker uses setPosition() method
            if (typeof mapMarker.setPosition === 'function') {
                mapMarker.setPosition({ lat, lng });  // google.maps.Marker (deprecated)
            } else {
                mapMarker.position = { lat, lng };    // AdvancedMarkerElement (recommended)
            }
        }
        ...
        if (injectedHandlers.renderItinerary) injectedHandlers.renderItinerary();
    });
}
...
export async function initMap() {
    ...
    if (container && window.google) {
        ...
        try {
            ...
            mapMarker = new AdvancedMarkerCtor({
                map: map,
                position: { lat, lng },
            });
        } catch (e) {
...
export async function renderRouteOnMap() {
    ...
    if (!AdvancedMarkerCtor && window.google?.maps?.importLibrary) {
        try {
            const { AdvancedMarkerElement } = await google.maps.importLibrary("marker");
            AdvancedMarkerCtor = AdvancedMarkerElement;
        } catch (e) {
            ...
            if (pos) {
                ...
                const getItemImage = async (targetItem) => {
                    ...
                    if (targetItem.memories && targetItem.memories.length > 0) {
                        ...
                        if (photo) {
                            const normalizedPhotoUrl = normalizeLegacyGooglePhotoUrl(photo.photoUrl, 400);
                            window.itemImageCache.set(cacheKey, normalizedPhotoUrl);
                            ...
                            const photoUrl = await new Promise((resolve) => {
                                service.findPlaceFromQuery(request, (results, status) => {
                                    if (
                                        ...
                                        resolve(buildGooglePhotoProxyUrl(results[0].photos[0], 400));
                                    } else {
                                        resolve(null);
                                    }
                                });
                    ...
                    // 3. Fallback: 기본 이미지
                    const fallback = normalizeLegacyGooglePhotoUrl(targetItem.image, 400)
                ...
                attachMarkerClickListener(marker, async () => {
                    ...
                    const safeImage = sanitizeUrl(normalizeLegacyGooglePhotoUrl(imgUrl, 400))
                        || 'https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=400&h=400&fit=crop';
                    ...
                    window.sharedInfoWindow.setContent(`
                        <div style="display: flex; min-width: 280px; max-width: 350px; cursor: pointer; overflow: hidden; font-family: 'MemomentKkukkukk', sans-serif; background: white;" onclick="window.viewTimelineItem(${i}, ${dIdx})">
                            <div style="flex: 1; padding: 12px; display: flex; flex-direction: column; justify-content: center; min-width: 150px;">
                                ...
                                <h4 style="margin: 0 0 4px 0; font-size: 16px; font-weight: bold; color: #333; line-height: 1.3;">${safeTitle}</h4>
                                <p style="margin: 0; font-size: 12px; color: #888; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; line-height: 1.4;">${safeLocation}</p>
                            ...
                            <div style="width: 100px; height: 100px; flex-shrink: 0; background-color: #f8f8f8;">
                                <img src="${safeImage}" style="width: 100%; height: 100%; object-fit: cover; transition: opacity 0.3s;" onerror="this.onerror=null;this.src='https://images.unsplash.com/photo-1488646953014-85cb44e25828?w=400&h=400&fit=crop';" />
                            </div>
                        </div>
                    `);
                    window.sharedInfoWindow.open({ map, anchor: marker });
...
function fillInAddress() {
    ...
    if (searchMode === 'trip') {
        ...
        if (place.photos && place.photos.length > 0) {
            ...
            if (photoUrl && window.updateMeta) window.updateMeta('mapImage', photoUrl);
            if (photoUrl && window.updateMeta) window.updateMeta('defaultMapImage', photoUrl);
        }
...
async function loadGoogleMapsAPI() {
    try {
        ...
        if (googleMapsUnavailableReason) {
            return false;
        }
        ...
        if (!mapsApiKey) {
            googleMapsUnavailableReason = "웹용 Google Maps API 키가 설정되지 않아 지도 기능을 비활성화합니다.";
            console.warn(googleMapsUnavailableReason);
            return false;
        }

        if (hasAttemptedGoogleMapsLoad) {
            return false;
        }

        hasAttemptedGoogleMapsLoad = true;

        const existingScript = document.querySelector('script[data-google-maps-loader="main-map"]');
        if (existingScript) {
            return false;
        }
        ...
        script.async = true;
        script.dataset.googleMapsLoader = 'main-map';
        script.onerror = () => {
            googleMapsUnavailableReason = "Google Maps JavaScript API 스크립트를 불러오지 못했습니다.";
            console.error(googleMapsUnavailableReason);
        };
