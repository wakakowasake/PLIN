...
- **여행 정보 수정 모달 고도화 (Trip Info UX Upgrade)**:
    - **롱프레스 동작 변경**: 모바일 환경에서 여행 제목/히로 영역 롱프레스 시 컨텍스트 메뉴 단계 없이 즉시 정보 수정 모달이 열리도록 개선.
    - **통합 상세 검색**: 기존 별도 모달로 분리되어 있던 장소 검색 기능을 여행 정보 수정 모달의 '제목' 필드에 직접 통합 (Google Autocomplete).
    - **좌표 및 이미지 자동 동기화**: 장소 검색 시 여행 제목뿐만 아니라 지도 좌표(`lat`, `lng`), 대표 이미지(Google/Unsplash)가 자동으로 업데이트되도록 연동 강화.
    - **모바일 규격 최적화**: 모바일 기기의 좁은 화면에서도 모달이 좌우 규격에 맞춰 안정적으로 표시되도록 레이아웃 및 반응형 너비 재조정.
