...
## [2.5.4] - 2026-01-22
### Added
- **배포 자동화 스크립트(`deploy:all`) 추가**:
    - 빌드, 템플릿 복사(동기화), 전체 배포를 한 번에 수행하는 명령어를 추가했습니다.
    - 공유 링크(`/v/:id`) 업데이트 시 발생할 수 있는 수동 동기화 누락 실수를 방지합니다.

### 22:45 - [AI] Human Error 방지를 위한 배포 자동화 스크립트 추가
- **변경 파일**: package.json, HISTORY.md, task.md
...
- **프로필 관리 기능 고도화**:
    ...
    - **보안 및 법적 고지 강화**: 프로필 하단에 보안 보증 문구와 이용약관/개인정보처리방침 링크를 추가했습니다.
    - **회원 탈퇴 기능**: 사용자가 자신의 계정과 데이터를 완전히 삭제할 수 있는 탈퇴 로직을 연동했습니다.

### 15:10 - [AI] 프로필 수정 모드 도입 및 보안/탈퇴 기능 강화
- **변경 파일**: public/index.html, public/js/ui/profile.js, public/js/ui/auth.js, HISTORY.md, task.md, walkthrough.md

---

## [2.4.3] - 2026-01-22
### Added
- **기존 사용자 필수 약관 동의 프로세스 도입**: 
    - 이미 로그인된 상태의 기존 사용자가 리뉴얼된 약관에 미동의 상태일 경우, 서비스를 이용하기 전 반드시 거쳐야 하는 '필수 동의 모달' 시스템을 구현했습니다.
    - 인증 감지 시 Firestore를 조회하여 `agreedToTerms` 여부를 실시간으로 검증하도록 로직을 강화했습니다.

### 14:55 - [AI] 기존 사용자 대상 필수 약관 동의 강제화 및 모달 연동
- **변경 파일**: public/index.html, public/js/ui/auth.js, HISTORY.md, task.md, walkthrough.md
...
- **로그인 전 약관 동의 필수 절차 도입**:
    - 로그인 화면(`login-view`)에 서비스 이용약관 및 개인정보처리방침 동의 체크박스와 백링크를 추가했습니다.
    - 체크박스 미동의 시 구글 로그인이 차단되도록 검증 로직을 강화했습니다.

### 14:45 - [AI] 로그인 약관 동의 UI 및 필수 검증 로직 구현
- **변경 파일**: public/index.html, public/js/ui/auth.js, HISTORY.md, task.md, walkthrough.md
...
## [2.4.0] - 2026-01-22
### Added
- **서비스 이용약관(`terms.html`) 신규 구축**: PLIN 서비스의 이용 조건, 회원 의무, 면책 조항 등을 명시한 공식 이용약관 페이지를 추가했습니다.
- **푸터(Footer) 고도화 및 법적 고지 통합**: 
    - 메인 페이지 및 공유 뷰어 하단에 이용약관 및 개인정보처리방침 링크가 포함된 통합 푸터를 적용했습니다.
    - 약관 페이지 상호 간 이동 링크를 추가하여 탐색 편의성을 높였습니다.

### 14:40 - [AI] 서비스 이용약관 신설 및 법적 고지 체계 통합
- **변경 파일**: public/terms.html, public/index.html, public/openview.html, public/privacy.html, HISTORY.md, task.md, walkthrough.md
